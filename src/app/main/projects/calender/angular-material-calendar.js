...
angular.module("materialCalendar").filter('trusted', function($sce){
...
angular.module("materialCalendar").directive("calendarMd", ["$compile", "$parse", "$http", "$q", "materialCalendar.Calendar", "MaterialCalendarData", function ($compile, $parse, $http, $q, Calendar, CalendarData) {
    ...
    var defaultTemplate = "<md-content layout='column' layout-fill md-swipe-left='next()' md-swipe-right='prev()'><md-toolbar><div class='md-toolbar-tools' layout='row'><md-button type='button' class='md-icon-button' ng-click='prev()' aria-label='Previous month'><md-tooltip ng-if='::tooltips()'>Previous month</md-tooltip>&laquo;</md-button><div flex></div><h2 class='calendar-md-title'><span>{{ calendar.start | date:titleFormat:timezone }}</span></h2><div flex></div><md-button type='button' class='md-icon-button' ng-click='next()' aria-label='Next month'><md-tooltip ng-if='::tooltips()'>Next month</md-tooltip>&raquo;</md-button></div></md-toolbar><!-- agenda view --><md-content ng-if='weekLayout === columnWeekLayout' class='agenda'><div ng-repeat='week in calendar.weeks track by $index' ><div ng-if='sameMonth(day)' ng-class='{&quot;disabled&quot; : isDisabled(day), active: active === day }  ng-click='handleDayClick(day);' ng-repeat='day in week' layout><md-tooltip ng-if='::tooltips()'>{{ day | date:dayTooltipFormat:timezone }}</md-tooltip><div>{{ day | date:dayFormat:timezone }}</div><div flex ng-bind-html='dataService.data[dayKey(day)]'></div></div></div></md-content><!-- calendar view --><md-content ng-if='weekLayout !== columnWeekLayout' flex layout='column' class='calendar'><div layout='row' class='subheader'><div layout-padding class='subheader-day' flex ng-repeat='day in calendar.weeks[0]'><md-tooltip ng-if='::tooltips()'>{{ day | date:dayLabelTooltipFormat }}</md-tooltip>{{ day | date:dayLabelFormat }}</div></div><div  style='height:100px;' ng-if='week.length' ng-repeat='week in calendar.weeks track by $index' ng-init='weekIndex = $index' flex layout='row'><div layout-align='center center' tabindex='{{ sameMonth(day) ? (day | date:dayFormat:timezone) : 0 }}' ng-repeat='day in week track by $index' ng-style='day.color' ng-click='handleDayClick(day);changeColor(day,dataService.data[dayKey(day)],weekIndex,$index);' flex layout='column' layout-padding ng-class='{&quot;disabled&quot; : isDisabled(day), &quot;active&quot;: isActive(day), &quot;md-whiteframe-12dp&quot;: hover || focus }' ng-focus='focus = true;' ng-blur='focus = false;' ng-mouseleave='hover = false' ng-mouseenter='hover = true'><md-tooltip ng-if='::tooltips()'> {{ day | date:dayTooltipFormat }}</md-tooltip><div layout='column' flex ng-style='day.textColor'> {{ day | date:dayFormat }}</div><div layout='column' flex style='padding-top: 0px;' compile='dataService.data[dayKey(day)]' flex ng-bind-html='dataService.data[dayKey(day)]  | trusted'></div></div></div></md-content></md-content>";
    ...
    return {
        ...
        link: function ($scope, $element, $attrs) {
            ...
            $scope.$on('close_calender',function(ex){ 
                var sampleObj = JSON.parse(localStorage.getItem("last_date"));
                console.log($scope.calendar.weeks[sampleObj.weekIndex][sampleObj.dayIndex]);

                $scope.calendar.weeks[sampleObj.weekIndex][sampleObj.dayIndex].textColor = {"color":"black"};
                $scope.calendar.weeks[sampleObj.weekIndex][sampleObj.dayIndex].color = {"background-color":"white"}; 
            ...
            $scope.changeColor = function (obj,date,weekIndex,dayIndex) { 
                
                obj.textColor = {"color":"white"};
                obj.color = {"background-color":"rgba(247, 56, 3, 0.92)"};
                ...
                localStorage.setItem("last_date", JSON.stringify(sampleObj));
            };
            ...
            for(var k in CalendarData.data){
                CalendarData.data[k] = "";
            }
            ...
            var getDayContent = function (date) {
                ...
                if ($scope.noCache) {
                    ...
                    // console.log($scope.dataService.data[getDayKey(date)]);
                    if($scope.dataService.data[getDayKey(date)] && $scope.dataService.data[getDayKey(date)] !== ""){                        
                        date.textColor = {"color":"white"};
                        date.color = {"background-color":"rgba(247, 56, 3, 0.92)"};
                    }
            ...
            $scope.$watch("dayContent",function(){
