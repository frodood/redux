(function ()
{
    ...
    angular
        .module('app.products')
        .controller('proEditController', proEditController);

    /** @ngInject */
    function proEditController($scope, $customCtrl,$rootScope,commonFact, $document, $mdDialog, $mdToast,$mdMedia, $mdSidenav, $state,settingSummary,uploaderService,$imageUploader,$serviceCall,$apis,$setUrl,$auth ,msSpinnerService)
    {
        var vm = this, 
            individualTaxes = [],
            multiplelTaxes = [];
            ...
            client.ifSuccess(function(data){
              var data = data.result
                if (ifArray(data)) {                    
                    ...
                    console.log(vm.fullProduct.productUnit);
                    vm.fullProduct.productUnit = data[0].productUnit;
                    vm.originalCode = data[0].productCode;
        ...
        function laodCusFieldArr(){
            var field = $customCtrl.getCustArr(); 
            field.settingCus(vm.proCustArr).appCus(vm.fullProduct.customFields);
            vm.fullProduct.customFields = field.fullArr().result;
        }

        function toggleChildStates(toggledState){
            $state.go(toggledState,{'itemID':vm.fullProduct.productCode});
        };
        ...
        function checkProductCode(){
            ...
            if (vm.originalCode === vm.fullProduct.productCode) {
                return true;
            }
        ...
        function saveEditProduct(){
            ...
            vm.spinnerService.show('pro-edit-spinner');
            ...
            client.ifSuccess(function(result){
                ...
                $mdToast.show(
                  $mdToast.simple()
                    .textContent('Product successfully edited')
                    .position('top right')
                    .hideDelay(3000)
                );
                ...
                vm.submitProgress = false;
                 ...
                 if (result.ID){
                    ...
                    localStorage.setItem("imgEditData", JSON.stringify(thumbnails));
                }
                vm.spinnerService.hide('pro-edit-spinner');
                $state.go("app.products.pro.detail",{'itemID' : vm.fullProduct.productCode})
                  // $state.go("home");
            })
            client.ifError(function(data){
                vm.spinnerService.hide('pro-edit-spinner');
